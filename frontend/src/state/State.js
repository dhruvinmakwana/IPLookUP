...
const useIPLookUPStore = create((set, get) => ({
    activeInput: 0, multiInputType: false, totalInputs: 4, ipInputs: ["", "", "", ""], bulkIpInputs: [],

    selectedIP: "", IPQueryResults: [], /**
     * Store selected IP
     * @param value
     */
    setSelectedIP: (value) => {
    ...
    setInputType: (value) => {
        set((state) => ({
            ...state, multiInputType: value, bulkIpInputs: [], ipInputs: ["", "", "", ""],
        }));
    }, /**
    ...
    fetchIPDetailsFromAPI: async (mode) => {
        ...
        try {
            ...
            set((state) => ({
                ...state, IPQueryResults: data.results, bulkIpInputs: "",
            }));
        } catch (e) {
            if (e?.message==="Network Error") {
                createToast("error", "Cannot reach server at the moment. Please try again later.");
            } else if (e?.response?.status === 429) {
                set((state) => ({
                    ...state, IPQueryResults: e.response.data.results, bulkIpInputs: "",
                }));
                ...
                createToast("error", e?.response?.statusText??"Something went wrong please try again later.");
            }
        }
    },
}));
